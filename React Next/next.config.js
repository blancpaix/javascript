...
// "build" : "ANALYZE=true NODE_ENV=production next build", 이렇게 리눅스나 맥에서만 가능,
// 하면됨 근데 이거는 문제가 윈도우에서 안됨 ㅋㅋㅋㅋ 리눅스나 맥에서는 되서
// 가능하게 하려면 npm i cross-env 설치를 해야 함 명령어 앞에서 cross-env 하나만 붙여주만 알아서 잘 돌아감
// 최종적으로는 cross-env ANALYZE=true NODE_ENV=production next build 이렇게 써야 함
...
module.exports = withBundleAnalyzer({
  ...
  webpack(config, { webpack }) {
    ...
    return {
      ...
      //실제로 immer 써도 됨 불변성 사용하려면 return produce(config, (draft) => {}) 뭐 이런식으로 쓰면 될듯?
      // module: {
      //   ...config.module,
      //   rules: [
      //     ...config.module.rules,
