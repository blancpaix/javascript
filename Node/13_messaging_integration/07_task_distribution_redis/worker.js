...
async function main() {
  await redisClient.xgroup('CREATE', 'tasks_stream', 'workers_group', '$', 'MKSTREAM')
    .catch(() => console.log(`Consumer group already exists`));

  const [[, records]] = await redisClient.xreadgroup(
    'GROUP', 'workers_group', consumerName, 'STREAMS', 'tasks_stream', '0'
  );
...
async function processAndAck(recordId, rawTask) {
  ...
  redisClient.xack('tasks_stream', 'workers_group', recordId);
};

main().catch(err => console.error(err));

/*
  redis 는 생각보다 알아야 할게 많네??
*/